	...
	public void testStartLimitVariation1() throws Exception {
		...
		try {
			...
			for (int i = 1; i <= 4; i++) {
				...
				assertWithMessage(exec, "Testing execution #" + i, expectedExitStatuses[i-1], exec.getExitStatus());
				...
				/////
				assertWithMessage(exec, "Testing execution #" + i, BatchStatus.FAILED, exec.getBatchStatus());

				List<StepExecution> steps = jobOp.getStepExecutions(lastExecutionId);
				if (i >= 1 && i <= 3) {
					assertWithMessage(exec, "Found 3 step executions", 3, steps.size());
				} else {
					assertWithMessage(exec, "Found 2 step executions", 2, steps.size());
				}
				for (StepExecution step : steps) {
					if (step.getStepName().equals("step1")) {
						assertWithMessage(exec, "Testing step execution #" + i + ", step1: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else if (step.getStepName().equals("step2")) {
						assertWithMessage(exec, "Testing step execution #" + i + ", step2: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else if (step.getStepName().equals("step3")) {
						assertWithMessage(exec, "Shouldn't have a StepExecution for step 3 on execution #4", i != 4);
						assertWithMessage(exec, "Testing step execution #" + i + ", step3: ", BatchStatus.FAILED, step.getBatchStatus());
					} else {
		...
		try {
			...
			for (int i = 1; i <= 7; i++) {
				...
				if (i == 1 || i == 2 || i == 5 || i == 6 || i == 7) {
					...
					assertWithMessage(exec, "Testing execution #" + i, BatchStatus.STOPPED, exec.getBatchStatus());
				}
				...
				if (i >= 1 && i <= 4 ) {
					...
					assertWithMessage(exec, "Found 1 step executions", 1, steps.size());
				} else if (i == 6 ) {
					assertWithMessage(exec, "Found 4 step executions", 4, steps.size());
				} else if (i == 7 ) {
					assertWithMessage(exec, "Found 2 step executions", 2, steps.size());
				}
				...
				for (StepExecution step : steps) {
					if (step.getStepName().equals("step1")) {
						assertWithMessage(exec, "Shouldn't have a StepExecution for step 1 on execution #5, i= " + i, i != 5);
						assertWithMessage(exec, "Testing step execution #" + i + ", step1: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else if (step.getStepName().equals("step2")) {
						assertWithMessage(exec, "Shouldn't have a StepExecution for step 1 on execution #5, i = " + i, i != 5);
						assertWithMessage(exec, "Testing step execution #" + i + ", step2: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else if (step.getStepName().equals("step3")) {
						assertWithMessage(exec, "Should only have a StepExecution for step 3 on executions #1,2,3, i= " + i, i <= 3);
						if (i <= 2) {
							assertWithMessage(exec, "Testing step execution #" + i + ", step3: ", BatchStatus.FAILED, step.getBatchStatus());
						} else {
							assertWithMessage(exec, "Testing step execution #" + i + ", step3: ", BatchStatus.COMPLETED, step.getBatchStatus());
						}
					} else if (step.getStepName().equals("step4")) {
						assertWithMessage(exec, "Should only have a StepExecution for step 4 on executions #4,5,6, i = " + i , i >= 4 && i <= 6);
						assertWithMessage(exec, "Testing step execution #" + i + ", step4: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else if (step.getStepName().equals("step5")) {
						assertWithMessage(exec, "Should only have a StepExecution for step 5 on execution #6, i = " + i , i == 6);
						assertWithMessage(exec, "Testing step execution #" + i + ", step5: ", BatchStatus.COMPLETED, step.getBatchStatus());
					} else {
						throw new IllegalStateException("Shouldn't get here, unknown step");
					}
				}
		...
		try {
			...
			for (int i = 1; i <= 3; i++) {
				...
				if (i == 1) {
					...
					assertWithMessage(exec, "Testing execution #" + i, BatchStatus.COMPLETED, exec.getBatchStatus());
				}

				List<StepExecution> steps = jobOp.getStepExecutions(lastExecutionId);
				if (i == 1) {
					assertWithMessage(exec, "Found 5 step executions", 5, steps.size());
				} else if (i == 2) {
					assertWithMessage(exec, "Found 1 step execution", 1, steps.size());
				} else {
					assertWithMessage(exec, "Found 4 step execution", 4, steps.size());
				}

				// All steps will be COMPLETED
				for (StepExecution step : steps) {
					assertWithMessage(exec, "Testing step execution #" + i + ", stepName = " + step.getStepName(), BatchStatus.COMPLETED, step.getBatchStatus());
				}
			}
		} catch (Exception e) {
			handleException(METHOD, e);
