...
public class ParallelExecutionTests {
	...
	public void testInvokeJobWithOnePartitionedStep() throws Exception {
		...
		try {
			...
			assertWithMessage(jobExecution, BatchStatus.COMPLETED, jobExecution.getBatchStatus());
		} catch (Exception e) {
			handleException(METHOD, e);
		}
	}
	...
	public void testInvokeJobWithOnePartitionedStepExitStatus() throws Exception {
		...
		try {
			...
			assertWithMessage(jobExecution, 1, stepExecutions.size());

			for (StepExecution stepEx : stepExecutions) {
				assertWithMessage(jobExecution, "STEP EXIT STATUS: 10", stepEx.getExitStatus());
				assertWithMessage(jobExecution, BatchStatus.COMPLETED, stepEx.getBatchStatus());
			}

			assertWithMessage(jobExecution, "JOB EXIT STATUS: 10", jobExecution.getExitStatus());
			assertWithMessage(jobExecution, BatchStatus.COMPLETED, jobExecution.getBatchStatus());
	...
	public void testInvokeJobSimpleSplit() throws Exception {
		...
		try {
			...
			assertWithMessage(execution, BatchStatus.COMPLETED, execution.getBatchStatus());
			assertWithMessage(execution, "COMPLETED", execution.getExitStatus());
		} catch (Exception e) {
			handleException(METHOD, e);
		}
	}
	...
	public void testPartitionedPlanCollectorAnalyzerReducerComplete() throws Exception {
		...
		try {
			...
			assertWithMessage(execution, "nullBeginCACACABeforeAfter", execution.getExitStatus());
	...
	public void testZeroBasedPartitionedPlanCollectorAnalyzerReducerRollback() throws Exception {
		...
		try {
			...
			Reporter.log("Execution exit status = " +  execution.getExitStatus()+"<p>");
			assertWithMessage(execution, "nullBeginCACACARollbackAfter", execution.getExitStatus());

			Reporter.log("Execution status = " + execution.getBatchStatus()+"<p>");
			assertWithMessage(execution, BatchStatus.FAILED,execution.getBatchStatus());
		} catch (Exception e) {
			handleException(METHOD, e);
		}
	...
	public void testPartitionedCollectorAnalyzerReducerChunkRestartItemCount10() throws Exception {
		...
		try {
			...
			{ // Use block to reduce copy/paste errors
				...
				assertWithMessage(execution1, "Testing execution #1", expectedExitStatus, execution1.getExitStatus());
			}

			{
				...
				assertWithMessage(execution2, "Testing execution #2 exit status", expectedExitStatus, execution2.getExitStatus());
				assertWithMessage(execution2, "Testing execution #2 batch status", BatchStatus.COMPLETED, execution2.getBatchStatus());
				assertWithMessage(execution2, "Testing execution #2 instance ID", execution1InstanceId, execution2.getInstanceId());
			}
    ...
    public void testPartitionedMapperOverrideFalseOnRestart() throws Exception {
        ...
        try {
            ...
            assertWithMessage(execution2, "nullBeginCABeforeAfter", execution2.getExitStatus());
    ...
    public void testPartitionedMapperOverrideTrueDiffPartitionNumOnRestart() throws Exception {
        ...
        try {
            ...
            Reporter.log("Execution exit status = " +  execution.getExitStatus()+"<p>");
            assertWithMessage(execution, "nullBeginCACARollbackAfter", execution.getExitStatus());
            ...
            JobExecution execution2 = jobOp.restartJobAndWaitForResult(execution.getExecutionId(), jobParams);
            Reporter.log("Execution exit status = " +  execution2.getExitStatus()+"<p>");
            assertWithMessage(execution2, "nullBeginCACACACARollbackAfter", execution2.getExitStatus());
    ...
    public void testPartitionedMapperOverrideTrueSamePartitionNumOnRestart() throws Exception {
        ...
        try {
            ...
            JobExecution execution2 = jobOp.restartJobAndWaitForResult(execution.getExecutionId(), jobParams);
            Reporter.log("Execution exit status = " +  execution2.getExitStatus()+"<p>");
            assertWithMessage(execution2, "nullBeginCACACARollbackAfter", execution2.getExitStatus());
