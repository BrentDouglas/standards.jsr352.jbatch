	...
	public void testTransitionElementOnAttrValuesWithRestartJobParamOverrides() throws Exception {
		...
		try {
			...
			// Create new block to facilitate copy-pasting without inadvertent errors
			{
				...
				assertWithMessage(execution1, "Testing execution #1", BatchStatus.STOPPED, execution1.getBatchStatus());
				assertWithMessage(execution1, "Testing execution #1", "STOPPED", execution1.getExitStatus());
			}

			{
				...
				assertWithMessage(execution2, "Testing execution #2", BatchStatus.FAILED, execution2.getBatchStatus());
				// See JSL snippet above for where "SUCCESS" comes from
				assertWithMessage(execution2, "Testing execution #2", EXECUTION2_EXPECTED_EXIT_STATUS_FROM_JSL_ATTRIBUTE, execution2.getExitStatus());
			}
	...
	public void testAllowStartIfCompleteRestartExecution() throws Exception {
		...
		try {
			...
			for (int i = 1; i <= 6; i++) {
				...
				assertWithMessage(exec, "Testing execution #" + i, "EXECUTION." + execString, exec.getExitStatus());
			}
		} catch (Exception e) {
			handleException(METHOD, e);
		}
